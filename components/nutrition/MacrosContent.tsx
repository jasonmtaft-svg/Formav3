...
type InnerTab = "calculator" | "foodlog";

// ── Component ─────────────────────────────────────────────────────────────────

export function MacrosContent() {
  const [tab, setTab] = useState<InnerTab>("calculator");
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      {/* Inner tab bar */}
      <div className="flex rounded-lg overflow-hidden border border-border-default m-4 mb-0">
        {(["calculator", "foodlog"] as InnerTab[]).map((t) => (
          <button
            key={t}
            type="button"
            onClick={() => setTab(t)}
        ...
        {tab === "foodlog" && (
          <div className="space-y-4">
            <div className="rounded-xl border border-border-default bg-surface p-4 space-y-3">
              ...
              {foodError && <p className="text-xs text-red-400">{foodError}</p>}
            </div>
